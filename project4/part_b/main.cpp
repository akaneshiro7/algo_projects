...
// dimension, i.e., they are each (BoardSize+1) * (BoardSize+1)
    ...
    int recursiveCalls; // New member to count recursive calls
...
bool board::solveBoard() {
    ...
    // Try filling the cell with all possible values
    for (int val = MinValue; val <= MaxValue; val++) {
        // Check if val not in conflict
        if (find(conflictVectors[i][j].begin(), conflictVectors[i][j].end(), val) == conflictVectors[i][j].end()) {
            ...
            clearCell(i, j); // Backtrack: Remove the value and try the next one
        }
    }
